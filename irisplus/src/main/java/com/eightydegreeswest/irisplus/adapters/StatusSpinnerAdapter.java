...
public class StatusSpinnerAdapter extends ArrayAdapter<String> {
    ...
    public StatusSpinnerAdapter(Context context, int resource, List<String> data) {
    ...
    public View getDropDownView(int position, View convertView, ViewGroup parent) {   // This view starts when we click the spinner.
        ...
        if (row == null) {
            ...
            row = inflater.inflate(R.layout.status_spinner, parent, false);
        }

        String item = data.get(position);

        if (item != null) {
            ImageView icon = (ImageView) row.findViewById(R.id.status_spinner_icon);
            TextView text = (TextView) row.findViewById(R.id.status_spinner_text);
            text.setText(item);
        }
